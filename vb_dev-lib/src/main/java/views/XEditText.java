...
public class XEditText extends AppCompatEditText {
    ...
    private void init() {
        ...
        if (mRightMarkerDrawable == null) { // didn't customize Marker
            if(null != getTag() && getTag().equals("white")) {
                mRightMarkerDrawable = getResources().getDrawable( R.drawable.icon_edit_clear_white);
            }else if(null != getTag() && getTag().equals("black")) {
                mRightMarkerDrawable = getResources().getDrawable(R.drawable.icon_edit_clear_black);
            }else if(null != getTag() && getTag().equals("blue")) {
                mRightMarkerDrawable = getResources().getDrawable(R.drawable.ic_edit_clear_blue);
            }else {
                mRightMarkerDrawable = ContextCompat.getDrawable(getContext(),
                        R.drawable.icon_edit_clear_black); // from support v4
...
//                    hideInputMethod();
    ...
    /**
     * 密码可见和不可见
     * @param visbleId  可见时drawable的Id
     * @param unVisibleId   不可见时drawable的Id
     */
    public void setPwdAble(final int visbleId, final int unVisibleId) {
        //自定义图标
        this.setCustomizeMarkerEnable(true);
        //图标显示状态
        this.setShowMarkerTime(XEditText.ShowMarkerTime.ALWAYS);
        //指定图标
        this.setRightMarkerDrawableRes(unVisibleId);
        //输入类型
        this.setInputType(InputType.TYPE_CLASS_TEXT |
                InputType.TYPE_TEXT_VARIATION_PASSWORD);
        //图标点击监听
        this.setOnMarkerClickListener(new XEditText.OnMarkerClickListener() {
            @Override
            public void onMarkerClick(float x, float y) {
                //改为不可见
                if (pwdHidden) {
                    XEditText.this.setRightMarkerDrawableRes(visbleId);
                    XEditText.this.setInputType(InputType.TYPE_CLASS_TEXT |
                            InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
                }
                //改为可见
                else {
                    XEditText.this.setRightMarkerDrawableRes(unVisibleId);
                    XEditText.this.setInputType(InputType.TYPE_CLASS_TEXT |
                            InputType.TYPE_TEXT_VARIATION_PASSWORD);
                }
                //切换时保持光标在最后
